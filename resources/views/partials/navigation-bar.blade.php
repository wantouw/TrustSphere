                ...
                <ul class="dropdown-menu">
                    @if (!Auth::user()->hasRole('admin'))
                        <li class="dropdown-item" style="border-bottom: 0.5px solid rgba(216, 216, 216, 0.837)">
                            ...
                            <a href="{{route('my_profile_page')}}">{{__('navbar.profile')}}</a>
                        </li>
                    @endif
                    <li class="dropdown-item">
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="w-100" style="text-align: left">{{__('navbar.logout')}}</button>
                        </form>
                    </li>
                </ul>
            @endif
